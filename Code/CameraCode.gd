...
#This enum lists all the possible states the camera can be in.
enum States {ROAD, SIDEWALK}

#This variable keeps track of the camera's current state.
var state: States = States.ROAD

@export var PlayerBody : CharacterBody3D
@export_range(1.0,10.0) var radius : float
@export_range(1.0,20.0) var speed : float = 10.0
